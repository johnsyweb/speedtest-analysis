...
- Or manually install Node.js **20.19+** or **22.12+** and **pnpm 10+** (see `packageManager` in `package.json`)
...
- **`.github/workflows/deploy.yml`** — builds the web application, uploads the Pages artifact, and deploys when the branch is `main`. It runs on pushes to `main` and on pull requests targeting `main`, so every proposed change goes through the same build before merge. Actions are pinned to **immutable commit SHAs** (with the corresponding release tag in a YAML comment); Dependabot can still open pull requests when newer SHAs are published.
